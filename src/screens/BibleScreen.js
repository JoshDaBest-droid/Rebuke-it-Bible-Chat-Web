  ...
  return (
    <View style={s.screen}>
      <View style={s.toolbar}>
        ...
        <Pressable style={s.arrowBtn} onPress={goPrev}><Text style={s.arrowText}>←</Text></Pressable>
        <Pressable style={s.arrowBtn} onPress={goNext}><Text style={s.arrowText}>→</Text></Pressable>
        <Pressable style={s.arrowBtn} onPress={toggleSpeak} accessibilityLabel={isSpeaking ? "Stop reading aloud" : "Read chapter aloud"}>
          <Text style={s.arrowText}>{isSpeaking ? "⏹" : "🔊"}</Text>
        </Pressable>
      </View>
      ...
      <FlatList
        ...
        ListFooterComponent={
          <View>
            <View style={s.navRow}>
              <Pressable style={s.navBtn} onPress={goPrev}><Text style={s.navBtnText}>← Previous Chapter</Text></Pressable>
              <Pressable style={s.navBtn} onPress={goNext}><Text style={s.navBtnText}>Next Chapter →</Text></Pressable>
            </View>

            <View style={s.card}>
              <Text style={s.badge}>SEARCH THE BIBLE</Text>
              ...
              {results.map(r => (
                <Pressable key={r.ref} style={s.verseCard} onPress={() => openVerseSheet(r.ref, r.text)}>
            ...
            <View style={s.card}>
              ...
              {bookmarks.map(b => (
                <Pressable key={"bm" + b.ref} style={s.verseCard} onPress={() => openVerseSheet(b.ref, b.text)}>
                  <Text style={s.verseCardRef}>Bookmark · {b.ref}</Text>
                  <Text style={s.verseCardText}>{b.text}</Text>
                </Pressable>
              ))}
              {highlights.map(h => (
                <Pressable key={"hl" + h.ref} style={s.verseCard} onPress={() => openVerseSheet(h.ref, h.text)}>
                  <Text style={s.verseCardRef}>Highlight · {h.ref}</Text>
                  <Text style={s.verseCardText}>{h.text}</Text>
                </Pressable>
              ))}
            </View>
          </View>
...
function makeStyles(c, textScale) {
  return StyleSheet.create({
    ...
    arrowText: { color: c.text, fontSize: 16 * textScale },
    ...
    navBtn: { borderWidth: 1, borderColor: c.border, borderRadius: 999, paddingVertical: 8, paddingHorizontal: 12 },
